 ...
 *port=MACH_PORT_NULL;
 if(__builtin_available(iOS 16.0, macOS 13.0, *)) {
  mach_port_options_t options={0};options.flags=MPO_REPLY_PORT;
  // Never fall back to an ordinary port on a kernel enforcing reply semantics.
  return mach_port_construct(mach_task_self(),&options,0,port);
 }
 *port=mach_reply_port();
 return MACH_PORT_VALID(*port)?KERN_SUCCESS:KERN_RESOURCE_SHORTAGE;
}
