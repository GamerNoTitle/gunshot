...
NSDictionary *GSNativeAccountSummary(void){assert(NSThread.isMainThread);return identity?@{@"identifier":identity}:nil;}
BOOL GSNativeAccountMatches(id value){assert(NSThread.isMainThread);return [value isEqual:identity];}
...
int main(void){@autoreleasepool{
 ...
 PHSUserItemsSynchronizer *a=[PHSUserItemsSynchronizer new];a.accountID=@"account-A";[a fetchData];
 PHSUserItemsSynchronizer *b=[PHSUserItemsSynchronizer new];b.accountID=@"account-B";[b fetchData];
 GSSetUploadHostForeground(YES);assert(requests==0); // Delayed native sign-in.
 identity=a.accountID;Poll();assert(a.fetches==1&&b.fetches==1); // Empty revision is not a completion.
 revision=1;Poll();Await(^BOOL{return a.fetches==2;});assert(b.fetches==1);
 ...
 identity=b.accountID;holdReply=NO;dispatch_semaphore_signal(releaseReply);
 ...
 identity=a.accountID;Poll();Await(^BOOL{return a.fetches==5;}); // Same revision, different current account.
 ...
 identity=nil;before=requests;GSSetUploadHostForeground(YES);assert(requests==before);
 ...
 assert(![json containsString:@"account-A"]&&![json containsString:@"account-B"]);
