...
3. Google Photos内のGoToHPを開くと `account_native` でこのアカウントを接続する。入力欄にトークンを貼り付ける必要はない。
...
- Google Photos内の設定を開くと、旧版/新版で利用可能なSSO APIからログイン中アカウントの認証を取得する。接続・更新操作でトークン入力を求めない。
